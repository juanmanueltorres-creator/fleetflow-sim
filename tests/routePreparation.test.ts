...
async function runPreparationWithLegDistances(legDistances: number[]) {
  const server = createServer((_request, response) => {
    response.writeHead(200, { 'content-type': 'application/json' })
    response.end(JSON.stringify({
      code: 'Ok',
      routes: [{
        legs: legDistances.map((distance) => ({ distance })),
        geometry: {
          type: 'LineString',
...
describe('offline route preparation', () => {
  it('accepts a zero-distance leg when the complete route still has positive distance', async () => {
    const { result, outputPath } = await runPreparationWithLegDistances([1200, 0, 1900])
  ...
  it('rejects a route whose total prepared distance is zero', async () => {
    const { result } = await runPreparationWithLegDistances([0, 0, 0])
